...
Текущая реализация: задачи описываются в БД, выполняются Celery-воркером, а запуск по расписанию доступен через Celery Beat либо встроенный планировщик (TASK_SCHEDULER_MODE=internal). Ограничение параллельных подключений задается переменной MIKROTIK_MAX_PARALLEL_CONNECTIONS.
